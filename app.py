...
APP_VERSION = "v15.5.37"
APP_DOCUMENTED_REVISIONS = 54
...
def _resolve_reference_profile_v15533(patient_id, record_id):
    """Prioridad: override de registro > perfil general del paciente."""
...
def _autonomic_abs_vector_levels_v15528(row, age_years=None, sex="No especificado"):
    ...
    # Regulación lenta · v15.5.36
    # L es un eje de INTENSIDAD/DESVIACIÓN, no una escala de calidad.
    # Con la normalización por rango preservado:
    #   L ≈ 0  -> dentro del rango / equilibrio lento preservado
    #   L < 0  -> expresión lenta reducida
    #   L > 0  -> expresión/carga lenta elevada
    # Por tanto, un L alto NO significa automáticamente "mejor".
...
def _autonomic_classify_instantaneous_v15531(
    ...
    if sum(finite) < 3 or not (np.isfinite(V_abs) and np.isfinite(S_abs)):
        return {
            ...
            "Score_Desacoplamiento_bruto": np.nan, "Desacoplamiento_permitido": False,
            "Contribucion_VS_pct": 55, "Contribucion_LCR_pct": 45,
            "Equilibrio_Lento_actual": np.nan,
            "Carga_Lenta_elevada_actual": False,
            "Expresion_Lenta_reducida_actual": False,
    ...
    # v15.5.36 · L no se interpreta como 'más alto = más funcional'.
    # El equilibrio lento es máximo cerca de L=0 (dentro del rango contextual).
    # L negativo aporta a reducción lenta; L positivo indica mayor carga/dominancia lenta.
    l_balance = float(np.clip(1.0 - abs(L_abs), 0.0, 1.0)) if np.isfinite(L_abs) else 0.5
    l_low = float(np.clip(-L_abs, 0.0, 1.0)) if np.isfinite(L_abs) else 0.0
    l_high = float(np.clip(L_abs, 0.0, 1.0)) if np.isfinite(L_abs) else 0.0

    lcr_functional = 0.25*l_balance + 0.40*pc + 0.35*pr
    lcr_reduced = 0.25*l_low + 0.35*nc + 0.40*nr
    ...
    # v15.5.33: la dispersión matemática NO define por sí sola desacoplamiento.
    # Se conserva como señal secundaria, pero requiere incoherencia fisiológica explícita.
    score_disc_raw = 0.60*dispersion + 0.40*sign_conflict
    score_disc = 0.20 * score_disc_raw
    ...
    explicit_discordance = bool(
        (vector_gap >= 0.50 and strong_alert_spec)
        or (vector_gap <= -0.50 and strong_vagal_spec)
        or (
            np.isfinite(R_abs) and np.isfinite(C_abs)
            and ((R_abs < -0.75 and C_abs > 0.50) or (R_abs > 0.50 and C_abs < -0.75))
        )
    )
    if explicit_discordance:
        score_disc = min(1.0, score_disc + 0.35)
    else:
        # Sin contradicción fisiológica explícita, desacoplamiento no puede ganar.
        score_disc = -0.5
    ...
    return {
        ...
        "Score_Desacoplamiento_bruto": float(score_disc_raw),
        "Desacoplamiento_permitido": bool(explicit_discordance),
        "Contribucion_VS_pct": 55,
        "Contribucion_LCR_pct": 45,
        "Equilibrio_Lento_actual": float(l_balance),
        "Carga_Lenta_elevada_actual": bool(l_high >= 0.50),
        "Expresion_Lenta_reducida_actual": bool(l_low >= 0.50),
...
with tab9:
    st.subheader("Índices fisiológicos · contextualización etaria · Regulación lenta = intensidad, no calidad")
    st.info(
    ...
    if _idx_mode_v15534 == "Contextualizado por edad/sexo":
        st.caption(
            "Modo ligero: la segunda capa se calcula sólo cuando la seleccionas, "
            "usa perfiles cargados una sola vez y cachea cada registro hasta que cambian sus datos o edad/sexo."
        )
        st.info(
        ...
        _indices_ctx_full_v15535 = _contextualize_indices_dataframe_v15535(indices_df, _profile_maps_v15535)
    ...
    if _indices_active_v15534.empty:
        ...
        if view_mode == "Registro seleccionado":
            ...
            if history_df_v140.empty:
                ...
                if _idx_mode_v15534 == "Contextualizado por edad/sexo":
                    hdf=_contextualize_indices_dataframe_v15535(hdf, _profile_maps_v15535)
...
with tab10:
    st.subheader("Control autonómico · regulación lenta equilibrada + perfiles por registro")
    st.info(
    ...
    if _auto_table_v15521.empty:
        ...
        with st.container(border=True):
            ...
            if np.isfinite(_vs_delta_v15537):
                if _vs_delta_v15537 >= 0.20:
                    _vs_explain_v15537 = "V supera a S: la relación vectorial apoya predominio vagal."
                elif _vs_delta_v15537 <= -0.20:
                    _vs_explain_v15537 = "S supera a V: la relación vectorial apoya predominio alerta/barorreflejo."
                else:
                    _vs_explain_v15537 = "V y S están próximos: el predominio depende de la coherencia espectral actual."
                st.caption(
                    _vs_explain_v15537 + " El resultado final también comprueba HF_DOM, LF_DOM y LF/HF actuales."
                )
        ...
        with st.expander("Historial cronológico completo", expanded=False):
            ...
            st.download_button(
                ...
                file_name='control_autonomico_v15537.csv',
                mime='text/csv'
            )
